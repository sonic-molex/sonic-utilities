...
def on_rpc(y_module: OrderedDict,
    ...
    def collect(block):
        if block is None:
            return []
        attrs = list()
        attrs.extend(get_leafs(block, grouping_name=''))
        attrs.extend(get_leaf_lists(block, grouping_name=''))
        attrs.extend(get_choices(y_module, block, conf_mgmt, grouping_name=''))
        attrs.extend(get_uses(y_module, block, conf_mgmt))
        return attrs
